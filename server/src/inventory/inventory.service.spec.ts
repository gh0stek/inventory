  ...
  describe('getStoreStats', () => {
    it('should return store statistics', async () => {
      ...
      mockDb.where.mockResolvedValueOnce([
        { totalProducts: 10, totalQuantity: '100', totalInventoryValue: '5000.00' },
      ]);
      // Out of stock count
    ...
    it('should use custom low stock threshold', async () => {
      ...
      mockDb.where.mockResolvedValueOnce([{ count: 2 }]);
      mockDb.where.mockResolvedValueOnce([{ count: 5 }]); // More items with higher threshold
      mockDb.orderBy.mockResolvedValueOnce([]);
    ...
    it('should correctly format category breakdown', async () => {
      mockDb.where.mockResolvedValueOnce([mockStore]);
      mockDb.where.mockResolvedValueOnce([
        { totalProducts: 3, totalQuantity: '30', totalInventoryValue: '300.00' },
      ]);
      mockDb.where.mockResolvedValueOnce([{ count: 0 }]);
      mockDb.where.mockResolvedValueOnce([{ count: 0 }]);
      mockDb.orderBy.mockResolvedValueOnce([
