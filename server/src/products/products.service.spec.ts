...
describe("ProductsService", () => {
  ...
  describe("findByStore", () => {
    ...
    it("should return paginated products for a store", async () => {
      ...
      // Products query
      mockDb.db.offset.mockResolvedValueOnce(products);
    ...
    it("should filter by category", async () => {
      ...
      mockDb.db.where.mockResolvedValueOnce([mockStore]);
      mockDb.db.where.mockResolvedValueOnce([{ count: 1 }]);
      mockDb.db.offset.mockResolvedValueOnce([mockProduct]);
    ...
    it("should return empty data when no products match filters", async () => {
      ...
      mockDb.db.offset.mockResolvedValueOnce([]);
  ...
  describe("findOne", () => {
    it("should return a product by id", async () => {
      mockDb.db.where.mockResolvedValue([mockProduct]);
  ...
  describe("create", () => {
    ...
    it("should create and return a new product", async () => {
      ...
      mockDb.db.where.mockResolvedValueOnce([mockStore]); // Store exists check
      mockDb.db.returning.mockResolvedValue([newProduct]);
  ...
  describe("update", () => {
    ...
    it("should update and return the product", async () => {
      ...
      mockDb.db.where.mockResolvedValueOnce([mockProduct]); // findOne call
      mockDb.db.returning.mockResolvedValue([updatedProduct]);
    ...
    it("should throw ConflictException on duplicate SKU", async () => {
      mockDb.db.where.mockResolvedValueOnce([mockProduct]);
      const error = { code: "23505" };
      mockDb.db.returning.mockRejectedValue(error);
